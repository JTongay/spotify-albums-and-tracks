...
function displayAlbumsAndTracks( event ) {
    ...
    var initialCall = $.ajax( {
        type: "GET",
        dataType: "json",
        url: albumUrl
    } )

    initialCall.done( function ( data ) {
        var allAlbums = data.items;
        allAlbums.forEach( function ( record ) {
            $.ajax( {
                type: "GET",
                dataType: "json",
                url: "https://api.spotify.com/v1/albums/" + record.id
            } ).done( function ( data ) {
                var albumArt = data.images[ 1 ].url;
                var trackList = data.tracks.items;
                var $img = $( '<img>' );
                var release = data.release_date;
                var title = data.name;
                var $albumTitle = $( "<h2>" );
                var $releaseDate = $( '<h3>Relase Date:' + release + "</h3>" )
                $img.attr( "src", albumArt )
                $albumTitle.text( title )
                appendToMe.append( $img )
                appendToMe.append( $albumTitle )
                appendToMe.append( $releaseDate )

                for ( var i = 0; i < trackList.length; i++ ) {
                    var $lists = $( '<ul>' );
                    var $titles = $( '<li>' );
                    $titles.text( trackList[ i ].track_number + ". " + trackList[ i ].name );
                    $lists.append( $titles );
                    appendToMe.append( $lists );
                }
            } )
        } )
    } )
